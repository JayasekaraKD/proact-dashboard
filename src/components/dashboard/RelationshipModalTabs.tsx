// src/components/dashboard/RelationshipModalTabs.tsx
...
const RelationshipModalTabs: React.FC = () => {
    const relation = useStore(selectedRelation);
    const isEdit = useStore(isEditMode);
    const { isUpdating, error } = useStore(modalState);

    const [formData, setFormData] = useState<Relation | null>(null);
    ...
    if (!relation || !formData) return null;
